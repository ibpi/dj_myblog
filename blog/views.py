...
def get_blogs_list_common_data(request, blogs_all_list):
    ...
    # 博客分类对应数量 BlogType.objects.annotate(blog_count=Count('blog'))
    blog_types = BlogType.objects.all()
    blog_types_list = []
    for blog_type in blog_types:
        blog_type.blog_count = blog_type.blog_count()
    ...
    for blog_date in blog_dates:
        blog_count = Blog.objects.filter(
            created_time__year=blog_date.year, created_time__month=blog_date.month
        ).count()
